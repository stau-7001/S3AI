...
from commons.utils import  update_matrix

class ParapredTrainer(Trainer):
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        super(ParapredTrainer, self).__init__(model, args, metrics, main_metric, device, tensorboard_functions,
...
#         print(Ab_ft.shape)
        reg_pred, cls_pred= self.model(Ab_ft.to(self.device),Ag_ft.to(self.device),torch.tensor([len(batch['H'][0]+batch['L'][0])]),torch.tensor([len(batch['spike'][0])])) # forward the sequence to the model
...
#         print(pred.shape, label.shape, label.float().unsqueeze(0).shape)
        loss = self.loss_func(reg_pred, cls_pred, reg_label.float().unsqueeze(0),cls_label.float().unsqueeze(0))
        return loss, reg_pred, cls_pred, reg_label.float(),cls_label.float().unsqueeze(0)
    ...
    def process_batch(self, batch, optim):
        loss, reg_pred, cls_pred, reg_label, cls_label = self.forward_pass(batch)
        cls_label = cls_label.clone().detach().squeeze(0)
        if optim != None:  # run backpropagation if an optimizer is provided (ie.train)
            ...
            self.optim_steps += 1
        return  loss, reg_pred.detach(), cls_pred.detach(), reg_label.detach(), cls_label.detach()
...
#             dicts = batch
            ...
            loss, reg_predictions, cls_predictions, reg_targets, cls_targets = self.process_batch(batch, optim)
            cur_res = {
                'loss': loss.detach(),
                'reg_predictions':reg_predictions.detach(),
                'cls_predictions':cls_predictions.detach(),
            }
#             dicts = {**dicts, **cur_res}
# #             print(dicts)
#             dict_list.append(dicts)
            with torch.no_grad():
                ...
                if self.optim_steps % self.args.log_iterations == 0 and optim != None:
                    if self.optim_steps % self.accumulation_steps == 0:# predict
                        metrics = self.evaluate_metrics(reg_accumulated_preds, cls_accumulated_preds, reg_accumulated_targets, cls_accumulated_targets)
                ...
                if optim == None and self.val_per_batch:  # during validation or testing when we want to average metrics over all the data in that dataloader
                    metrics_results = self.evaluate_metrics(reg_accumulated_preds, cls_accumulated_preds, reg_accumulated_targets, cls_accumulated_targets, val=True)
        ...
        if optim == None:
            if self.val_per_batch:
                ...
                total_metrics = self.evaluate_metrics(reg_epoch_predictions, cls_epoch_predictions, reg_epoch_targets, cls_epoch_targets, val=True)
...
#                 json.dump(converted_dict_list, outfile)
    ...
    def evaluate_metrics(self, reg_predictions, cls_predictions, reg_targets, cls_targets, batch=None, val=False) -> Dict[str, float]:
        metrics = {}
