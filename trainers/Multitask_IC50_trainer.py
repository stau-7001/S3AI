...
from commons.utils import flatten_dict, tensorboard_gradient_magnitude, move_to_device, update_matrix
import json
...
class MultitaskIC50Trainer(Trainer):
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        super(MultitaskIC50Trainer, self).__init__(model, args, metrics, main_metric, device, tensorboard_functions,
                                                               optim, main_metric_goal, loss_func, scheduler_step_per_batch)
        
    def forward_pass(self, batch):
#         print(batch[0])
        reg_pred, cls_pred, combined_representation = self.model(batch['H'],batch['L'],batch['spike'])  # forward the sequence to the model
...
#         print(pred.shape, label.shape, label.float().unsqueeze(0).shape)
        loss = self.loss_func(reg_pred, cls_pred, reg_label.float().unsqueeze(0),cls_label)
        # print(reg_pred, cls_pred, reg_label,cls_label)
        return loss, reg_pred, cls_pred, reg_label.float().unsqueeze(0),cls_label,combined_representation
