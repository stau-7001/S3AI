...
class RegTrainer(Trainer):
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        super(RegTrainer, self).__init__(model, args, metrics, main_metric, device, tensorboard_functions,
                                                               optim, main_metric_goal, loss_func, scheduler_step_per_batch)
        
    def forward_pass(self, batch):
#         print(batch[0])
        reg_pred = self.model(batch['H'],batch['L'],batch['spike'])  # forward the sequence to the model
