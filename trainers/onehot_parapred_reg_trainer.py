...
class OnehotRegParapredTrainer(Trainer):
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        super(OnehotRegParapredTrainer, self).__init__(model, args, metrics, main_metric, device, tensorboard_functions,
...
#         print(batch[0])
        ...
        reg_pred= self.model(Ab_ft.to(self.device),Ag_ft.to(self.device),torch.tensor([len(batch['H'][0]+batch['L'][0])]),torch.tensor([len(batch['spike'][0])]))  # forward the sequence to the model
