...
import pandas as pd
import torch
from commons.dataset_tools import get_padding_ft_dict
from commons.k_mer_utils import KmerTranslator
try:
...
def amino_seq_to_one_hot(protein_seq, protein_type, antibody_max_len= 230, virus_max_len=1275):
    assert protein_type in ['virus', 'antibody']
    if protein_type == 'virus':
        max_sql_len = virus_max_len
    else:
