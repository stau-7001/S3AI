...
def get_mask_output(args, model, activation, Ab_seq_H, Ab_seq_L, CDR_H, CDR_L):
    ...
    c_num = len(combine_CDR)
    mask_cdr = torch.BoolTensor(generate_all_masks(c_num))

    inner_v_S = []
    for m_cdr in tqdm(mask_cdr):
        ...
        with torch.no_grad():
            ...
            for i in range(c_num):
                if m_cdr[i] == False:
                    x_act[:,:,:,combine_CDR[i][0]:combine_CDR[i][1]+1] = 0
                ...
                if m[i] == False:
                    if player[i][0]!=-1:
                        ...
                        for j in range(c_num):
                            if m_cdr[j] == True:
                                x_act[:,:,:,combine_CDR[j][0]:combine_CDR[j][1]+1] = 0
        ...
        inner_v_S.append(phi[-1])
    
    inner_phi = []
    for i in range(c_num):
        inner_phi_i = 0.0
        for j, m in enumerate(mask_cdr):
            if m[i] == True: continue
            C_S = torch.sum(m)
            w_i = 1.0
            for k in range(1,c_num-C_S):
                w_i = w_i*k/(C_S+k)
            w_i = w_i/c_num
            inner_phi_i += w_i*(inner_v_S[j+(1<<(c_num-i-1))]-inner_v_S[j])
        inner_phi.append(inner_phi_i.item()) 
    print('inner_phi',inner_phi)
    return inner_v_S[0], inner_phi, player
...
if __name__ == '__main__':
    ...
    for i, batch in enumerate(tqdm(train_loader)):
        ...
        ic50 = batch['IC50']
        Ag_seq = batch['spike']
        H_list.append(Ab_seq_H)
        L_list.append(Ab_seq_L)
        ...
        v_empty, v_N = v_empty, v_N.cpu().numpy()
